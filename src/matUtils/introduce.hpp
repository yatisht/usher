...
float get_association_index_new(MAT::Tree* T, std::map<std::string, float> assignments, MAT::Node* subroot = NULL);
float get_association_index(MAT::Tree* T, std::map<std::string, float> assignments, MAT::Node* subroot = NULL);
std::vector<std::string> find_introductions(MAT::Tree* T, std::vector<std::string> samples, std::vector<std::string> regions, bool add_info, std::string clade_output, float min_origin_confidence); 
