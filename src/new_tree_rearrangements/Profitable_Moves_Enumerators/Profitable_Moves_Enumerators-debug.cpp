...
int get_parsimmony_score_only(MAT::Node* src, MAT::Node* dst,MAT::Node* LCA){
            ...
            MAT::Node* actual_LCA=get_mutation_path(mutations, src, dst, path);
            mutations.merge(actual_LCA->mutations, MAT::Mutations_Collection::KEEP_SELF);
            ...
            for(size_t idx=0;idx<mutations.size();idx++){
                ...
                int this_change=new_parsimony_score-original_mutation_count[idx].count;
                if(this_change){
                    fprintf(stderr,"change of %d @ %d \n",this_change,position);
                    for(auto& temp:original_mutation_count[idx].mut_count){
                        MAT::Node* corresponding_node_in_new_tree=new_tree.get_node(temp.first);
