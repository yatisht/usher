...
// Commented out on usher.cpp
// VCF reader called with a mapper input
bool Mutation_Annotated_Tree::read_vcf (Mutation_Annotated_Tree::Tree* T, boost::iostreams::filtering_istream &instream, mapper_input &inp, std::vector<Node*> &bfs, std::unordered_map<std::string, size_t>& bfs_idx, std::vector<Missing_Sample>& missing_samples, bool &header_found, std::vector<std::string> &variant_ids) {
