...
bool decrement_sensitive(uint8_t decrement,uint8_t major_allele,uint8_t boundary_allele, int parent_increment_effect, int parent_decrement_effect){
    auto not_decremented=major_allele&(~decrement);
    if (not_decremented) {
        //No Par score change at this position, but did remove major alleles that will increase parsimony score of ancestors?
        uint8_t decremented_major_allele=major_allele&decrement;
        return parent_decrement_effect&(1<<decremented_major_allele);
    }
    //decremented all major allele. see whether some new major alllel from boundary alleles can offset
    return !(parent_increment_effect&(1<<(boundary_allele&(~decrement))));
}

bool increment_sensitive(uint8_t increment,uint8_t major_allele,uint8_t boundary_allele, int parent_increment_effect, int parent_decrement_effect){
    auto major_allele_incremented=increment&major_allele;
    if (major_allele_incremented) {
        //Reduced parsimony score at this node, make sure not decrease it at parent node from removal of major allele not decremented
        return !(parent_decrement_effect&(1<<(major_allele&(~increment))));
    }
    //Incrementing boundary allele, no par score change at this node, but may parent
...
static std::pair<uint16_t,uint16_t> update_sensitve_allele( int parent_decrement_effect, int parent_increment_effect,
                                   MAT::Mutation &out) {
    ...
    for (uint32_t ptr=1; ptr<15; ptr++) {
        increment_effect|=(increment_sensitive(ptr, major_allele, boundary_allele, parent_increment_effect, parent_decrement_effect)<<ptr);
    }
    for (uint32_t ptr=1; ptr<15; ptr++) {
        decrement_effect|=(decrement_sensitive(ptr, major_allele, boundary_allele, parent_increment_effect, parent_decrement_effect)<<ptr);
    }
    out.set_sensitive_change(move_bits(decrement_effect),move_bits(increment_effect));
    return std::make_pair(decrement_effect, increment_effect);
}
static std::pair<uint16_t,uint16_t> update_sensitve_allele(const Sensitive_Alleles &par_mut,
                                   MAT::Mutation &out) {
    return update_sensitve_allele(par_mut.decrement_effect,
                           par_mut.increment_effect, out);
}
static std::pair<uint16_t,uint16_t> update_sensitve_allele(MAT::Mutation &out) {
    auto par_idx=one_hot_to_two_bit(out.get_par_one_hot());
    return update_sensitve_allele(default_decrement_effect[par_idx], default_increment_effect[par_idx], out);
}
static void filter_output(const MAT::Mutation &mut,std::pair<uint16_t,uint16_t> effect,std::vector<Sensitive_Alleles>& out) {
    auto mut_idx=one_hot_to_two_bit(mut.get_mut_one_hot());
    if (effect.first!=default_decrement_effect[mut_idx]||effect.second!=default_increment_effect[mut_idx]) {
        out.push_back(Sensitive_Alleles{mut.get_position(),effect.first,effect.second});
    }
}
struct Walker : public tbb::task {
    ...
    Walker(MAT::Node *root) : root(root) {}
    ...
    tbb::task *execute() override {
        ...
        for (auto child : root->children) {
            if (child->is_leaf()) {
                ...
                auto child_task =
                    new (continuation->allocate_child()) Walker(child);
...
void adjust_all(MAT::Tree &tree) {
    ...
    init();
    auto task_root = new (tbb::task::allocate_root()) Walker(tree.root);
    ...
    fprintf(stderr,"Updating sensitive alleles take %ld minutes",
...
static void check(MAT::Node* start_node, nuc_one_hot incremented,const MAT::Mutation& mut){
    ...
    while (start_node) {
        ...
        if (iter==start_node->mutations.end()) {
            ...
            int change=0;
            decrement_increment_mutation_count(*iter, mut_count_change,ignored,change);
...
static void check_each_node(MAT::Node* node){
    for (const auto& mut : node->mutations) {
        uint8_t nuc_to_check=mut.get_all_major_allele()|mut.get_boundary1_one_hot();
...
int main(int argc,char** argv){
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0,bfs_ordered_nodes.size()),checker{bfs_ordered_nodes});
}