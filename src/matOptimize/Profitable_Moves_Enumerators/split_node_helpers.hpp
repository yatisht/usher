...
template<typename T>
static bool add_node_split(const T& pos,nuc_one_hot ori_major_allele,nuc_one_hot allele1,nuc_one_hot allele2, Mutation_Count_Change_Collection& out,int& par_score){
    bool not_shared=false;
...
static bool add_node_split(const MAT::Mutation& mut,Mutation_Count_Change_Collection& out,int& par_score){
...
static bool add_node_split(const Mutation_Count_Change& mut,Mutation_Count_Change_Collection& out,int& par_score){
    return add_node_split(mut,mut.get_par_state(),mut.get_par_state(),mut.get_incremented(),out,par_score);
}
//at both
static bool add_node_split(const MAT::Mutation& mut, nuc_one_hot allele1, nuc_one_hot allele2,Mutation_Count_Change_Collection& out,int& par_score){
