...
static int leader_thread(
    ...
    if (options.tree_in=="") {
        ...
        if (process_count==1) {
            ...
            MPI_reassign_states(tree, position_wise_out, samples_to_place.front().sample_idx,true);
        }
    ...
    sort_samples(options, samples_to_place, tree);
    ...
    while (true) {
        ...
        if (optimization_radius > 0) {
            leader_thread_optimization(tree, position_wise_out, curr_idx, optimization_radius, samples_to_place[0].sample_idx, ignored_file);
        }
        if (curr_idx<samples_to_place.size()) {
            int to_board_cast=0;
            if (process_count>1) {            
...
int main(int argc, char **argv) {
    ...
    ("sort-before-placement-1,s", po::bool_switch(&options.sort_before_placement_1), \
    ...
    ("sort-before-placement-2,S", po::bool_switch(&options.sort_before_placement_2), \
    ...
    ("write-uncondensed-final-tree,u", po::bool_switch(&options.out_options.print_uncondensed_tree), "Write the final tree in uncondensed format and save to file uncondensed-final-tree.nh in outdir")
    ...
    ("write-parsimony-scores-per-node,p", po::bool_switch(&options.print_parsimony_scores), \
    ...
    ("retain-input-branch-lengths,l", po::bool_switch(&options.out_options.retain_original_branch_len), \
    ...
    return leader_thread(
        ...
        if (options.sort_before_placement_1||options.sort_before_placement_2) {
            ...
            assign_levels(tree.root);
            set_descendant_count(tree.root);
            follower_place_sample(tree,100,false);        
        }
        while (true) {
