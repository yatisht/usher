...
UShER started with a single program but has since evolved into a package consisting the following programs for rapid phylogenetic analyses, particularly suitable for the SARS-CoV-2 genomes. 
...
* **RIPPLES** is a program that uses a phylogenomic technique to detect recombinant nodes and their ancestors in a mutation-annotated tree (MAT).  
